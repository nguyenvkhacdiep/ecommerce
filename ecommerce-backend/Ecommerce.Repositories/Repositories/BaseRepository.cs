...
public class BaseRepository<T> : IBaseRepository<T> where T : class
{
    ...
    {
        return await _context.Database.SqlQuery<T1>(FormattableStringFactory.Create(query)).ToListAsync();
    }

    public async Task<int> GetCountByQueryAsync<T1>(string query)
    {
        return await _context.Database.SqlQuery<T1>(FormattableStringFactory.Create(query)).CountAsync();
    }
}